 ...
 *  @project Fetch
 ******************************************************************************/      
class FetchResponse extends FetchBase {
  ...
  final public function __construct($body,$response_info,$headers){
    ...
    $http_header = $this->field_map['headers']['http'];
    if(is_array($http_header)){ $http_header = end($http_header); }//if
    
    $this->field_map['status'] = $http_header;
    
    $response_list = explode(' ',$http_header,3);
  ...
  /**
   *  pass in one or more codes to see if any of those was the response code
   *
   *  @since  2-11-11   
   *  @param  integer $args,... one or more codes
   *  @return boolean   
   */
  public function isCode()
  {
    $code_list = func_get_args();
    return in_array($this->getCode(),$code_list,true);
