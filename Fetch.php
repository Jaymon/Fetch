...
class Fetch extends FetchBase {
  ...
  final public function __construct($url = ''){
    ...
    $this->setUserAgent('Mozilla/4.0 (compatible; MSIE 5.01; Windows NT 5.0) Fetch');
  ...
  public function start(){
    ...
    if($this->isMethod(self::METHOD_POST)){
      ...
      // if you use CURLOPT_POST then any redirects will be switched to a GET...
      curl_setopt($curl_handler, CURLOPT_POST, 1);
      
      // if you use CUSTOMREQUEST then any redirects will stay a POST but won't send the POST body...
      ///curl_setopt($curl_handler, CURLOPT_CUSTOMREQUEST,self::METHOD_POST);
      
      // see: http://bugs.php.net/49571 for discussion
    ...
    if(!empty($this->field_map['username']) && isset($this->field_map['password'])){
      ...
      curl_setopt($curl_handler, CURLOPT_UNRESTRICTED_AUTH, true);
      curl_setopt(
        $curl_handler, 
        CURLOPT_USERPWD, 
   ...
   *      
   *  @return FetchResponse  the response object containing what was downloaded
   *                          from the url
   */
    ...
    do{
      ...
      if($active_curl_count !== $current_curl_count){
        ...
        while($done_map = curl_multi_info_read($this->mcurl_handler)){
          ...
          $ret_instance = new FetchResponse(
            $response,
            $response_info,
  ...
  public function setHeaders($val){
    ...
    foreach($val as $header_name => $header_val){
      ...
      $header_val = (string)$header_val;
    
      // header is complete, eg: "header_name: header_val" so break it up...
      if(ctype_digit($header_name)){
      
        $header_bits = explode(':',$header_val,2);
   ...
   *  
   *  note: if you are submitting raw fields, then they aren't encoded or anything, it is
   *  up to you to escape appropriately
   *      
  ...
  protected function encodeFields($fields,$separator = '&'){
    ...
    return is_array($fields) ? http_build_query($fields,'',$separator) : $fields;
